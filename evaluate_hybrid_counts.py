...
LOC_THRESHOLD = 0.5    # 0.5
NMS_KERNEL = 3  # must be odd     3, 7
...
def evaluate_counts_by_bins(
    ...
    """Evaluate counting metrics per bin for both density‑ and localization‑based counts.
    ...
    Returns:
        Dictionary with keys 'pos' and 'neg', each containing a list (one per bin) of
        dictionaries with metrics for 'density' and 'localization'.
    """
...
def main():
    ...
    # Evaluate counts by bins
    results = evaluate_counts_by_bins(
        model=model,
        loader=loader,
        bins=COUNT_BINS,
        device=DEVICE,
