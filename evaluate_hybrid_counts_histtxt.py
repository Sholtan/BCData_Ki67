...
def evaluate_counts_by_bins(
    ...
    Returns:
        A tuple of two dictionaries:
          1) A metrics dictionary keyed by class ('pos', 'neg') and containing lists (one per bin)
             of metric dictionaries for 'density' and 'localization' strategies with MAE, RMSE, MAPE,
             and sample count.
          2) A parallel errors dictionary keyed by class and strategy containing lists of raw
             (prediction - ground_truth) errors for each bin.
    """
    ...
    # Initialize per‑sample error storage for bias diagnostics
    errors: Dict[str, Dict[str, List[List[float]]]] = {
        cls: {strategy: [ [] for _ in bins ] for strategy in ['dens', 'loc']} for cls in ['pos', 'neg']
    }
        ...
        for img, _loc_hm, _count_hm, pos_pts, neg_pts in loader:
            ...
            for i, (gt_pos, gt_neg) in enumerate(gt_tensor):
                ...
                if bin_pos >= 0:
                    ...
                    # Append raw errors for positive samples
                ...
                if bin_neg >= 0:
                    ...
                    # Append raw errors for negative samples
...
def main():
    ...
    # Evaluate counts by bins and collect raw error lists
    ...
    try:
        ...
        plt = None
    # If matplotlib is available, generate histogram plots and text files
    if plt is not None:
        for cls in ['pos', 'neg']:
            for bin_idx, (lo, hi) in enumerate(COUNT_BINS):
                for strategy_key, strategy_name in [('dens', 'density'), ('loc', 'localization')]:
                    ...
                    if not errs:
                        continue
                    # Plot histogram using default styling; do not specify colors
                    plt.figure()
                    ...
                    img_name = f"hist_{cls}_{strategy_key}_bin{lo}_{hi_str}.png"
                    img_path = hist_dir / img_name
                    ...
                    with open(txt_path, 'w') as f:
                        for i in range(len(counts)):
                            ...
                            f.write(f"{start:.4f}-{end:.4f} > {counts[i]}\n")
        print(f"\nHistogram images saved under: {hist_dir.resolve()}")
        print(f"Histogram data saved under: {hist_txt_dir.resolve()}")
